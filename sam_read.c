...
int bam1_site_base(const bam1_t *b, int32_t ref, hts_pos_t pos, 
    ...
    hts_pos_t right_pos = bam_endpos(b); // position after last base that consumes reference.
...
int bam1_sites_bases(const bam1_t *b, int32_t *ref, hts_pos_t *pos, 
        uint8_t **base, uint8_t *qual){
    return(0);
}
